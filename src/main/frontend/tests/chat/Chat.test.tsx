...
import {
  ...
  Message,
  Subscription,
} from "../../../resources/META-INF/resources/frontend/chat/Chat.js";
...
const getAiChatServiceMock = (
  ...
  return {
    stream: vi
      .fn()
      .mockReturnValue(
        getSubscriptionMock() as unknown as Subscription<string>
      ),
...
const getInputElement = () =>
  screen
    .getByLabelText<HTMLTextAreaElement>("Message input")
    .querySelector("textarea")!;

const getSendButton = () => screen.getByLabelText<HTMLButtonElement>("Send");

const getUploadButton = () =>
  screen.getByLabelText<HTMLButtonElement>("Upload attachment");
...
describe("Chat", () => {
  ...
  it("renders placeholder while waiting for assistant", async () => {
    ...
    expect(
      screen.getByLabelText("Chat messages").querySelector(".waiting-message")
    ).to.exist;
  });
  ...
  it("hides Vaadin loading indicator while waiting", async () => {
    ...
    expect(
      screen.queryByText("Vaadin loading indicator")?.offsetHeight
    ).to.equal(0);
  });
